...
async function startRoom(words?: string) {
  ...
  const put = async (slot: string, part: string, blob: string) => {
    const r = await fetch(`${base()}/invite/${slot}?part=${part}`, {
      method: "POST",
  ...
  const get = async (slot: string, part: string, waitMs = 0): Promise<string | null> => {
    ...
    for (;;) {
      const r = await fetch(`${base()}/invite/${slot}?part=${part}`).catch(() => null)
  ...
  if (joining) {
    ...
    const key = pake.finish(mine, theirs, inv.slot, "crosstalk/room/v1")
...
Waiting…`)
  ...
  const key = pake.finish(mine, theirs.slice(0, dot), slot, "crosstalk/room/v1")
...
async function mute() {
  ...
  console.log(
    r.mutedUntil
      ? `muted ${peer ?? `all ${r.muted.length} peer(s)`} until ${new Date(r.mutedUntil).toLocaleTimeString()}`
      : `unmuted ${peer ?? "everyone"}`,
  )
}
...
async function doctor() {
  ...
  rows.push([
    "inbox socket",
    socket ? fs.existsSync(socket) : null,
  ...
  rows.push([
    ...
    process.env.CLAUDE_CODE_MESSAGING_TOKEN ? "present" : socket
      ? "missing, messages arrive as anonymous peers"
      : "only used with a Claude Code inbox socket",
  ])
  ...
  // Nothing reports an error when that happens; the message simply never lands.
  {
    ...
    try {
      // Separate CROSSTALK_HOME identities are valid. Count owners of this
      // socket, including an older listener whose socket was unlinked.
      const output = execFileSync("lsof", ["-nP", "-U", "-Fpn"], {
      ...
      for (const line of output.split("\n")) {
        ...
        if (line === `n${P.daemonSock}` && pid) owners.add(pid)
      }
      count = owners.size
    } catch {}
    rows.push([
      ...
      !live
        ...
        : count !== undefined && count > 1
          ? `${count} processes hold ${P.daemonSock}; duplicate daemons can lose messages`
          : `running for ${ROOT}${count === undefined ? "; duplicate check unavailable" : ""}`,
    ])
  }
  ...
  if (!verb || verb === "list") {
    ...
    for (const room of r.rooms) {
      const who = room.members
        .map((m: any) => m.label + (m.you ? " (you)" : "") + (m.state === "invited" ? " (invited)" : "") + (!m.direct && !m.you ? " ·no direct channel" : ""))
        .join(", ")
      if (room.pending) {
...
/** Anything on this machine can put a line on your screen without a room. */
async function post() {
...
and a message reaches whichever machine you are sitting at.`)
  ...
  if (!id) die("nothing to link yet. Run /crosstalk:room new first, or run this on the machine that already has your identity.")
